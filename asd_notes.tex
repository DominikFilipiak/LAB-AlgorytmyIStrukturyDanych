...
Strona WWW: \url{http://dfilipiak.com}, e-mail: \url{dominik.filipiak@standardowauczelnianadomena} (proszę zaczynać tytuły wiadomości od [ASD]).
...
\section{Wstęp do analizy algorytmów}
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1] % The number tells where the line numbering should start
        \Function{InsertionSort}{$A$}  \algcost{koszt}{krotność}
            	\For{$j \gets 1$ \textbf{to} $A$.$length$}  \algcost{$c_1$}{$n$}
            		\State $key \gets A[j]$ \algcost{$c_2$}{$n-1$}
            		\State // Wstaw $A[j]$ w posortowany ciąg $A[1 \ldots j-1]$ \algcost{$c_3$}{$0$}
            		\State $i \gets i - 1$ \algcost{$c_4$}{$n-1$}
            		\While{$i > 0$ \textbf{and} $A[i] > key$} \algcost{$c_5$}{$\sum_{j=1}^{n} \left ( t_{j} \right )$}
            			\State $A[i+1] \gets A[i]$ \algcost{$c_6$}{$\sum_{j=1}^{n} \left ( t_{j} - 1\right )$}
            			\State $i \gets i - 1$ \algcost{$c_7$}{$\sum_{j=1}^{n} \left ( t_{j} - 1 \right )$}
            		\EndWhile
