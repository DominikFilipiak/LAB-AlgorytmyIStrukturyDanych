...
Tworzenie drzewa:
\begin{minted}{python}
# drzewo z CLRS, str. 288, rys. 12.1a.

n_two = Node(2, None, None)
n_five = Node(5, None, None)
n_five_2 = Node(5, n_two, n_five)
n_eight = Node(8, None, None)
n_seven = Node(7, None, n_eight)
n_six = Node(6, n_five_2, n_seven)

n_six.parent = None
n_five_2.parent = n_six
n_five.parent = n_five_2
n_two.parent = n_five_2
...
\begin{figure}[!htpb]
	\centering
	\includegraphics[width=.7\linewidth]{figures/tree_traversal}
	\caption{Przechodzenie po drzewie. Źródło: \protect\url{http://geeksforgeeks.com}}
\end{figure}
...
def treeSuccessor(x):
  ...
  while y is not None and x == y.right:
    ...
    y = y.parent
  return y
print(treeSuccessor(tree.root)) # inorder
\end{minted}
...
treeInsert(tree, Node(4, None, None))
...
\begin{algorithm}
    ...
    \label{alg:huffman}
    \begin{algorithmic}[1] % The number tells where the line numbering should start
        \Function{Huffman}{$C$} \algcost{}{// $O(n \log n)$}
        		\State $n \gets |C|$
        		\State $Q \gets \Call{PriorityQueue}{C}$ \algcost{}{// Kolejka priorytetowa}
...
\begin{figure}[h]
	...
	\end{tikzpicture}
	\caption{Drzewo odpowiadające kodom}
\end{figure}
...
\begin{algorithm}
    ...
    \label{alg:bfs}
    \begin{algorithmic}[1] % The number tells where the line numbering should start
        \Function{BFS}{$G, s$} \algcost{}{//$O(|V|+|E|)$,  $s$ -- start}
        		\For{ \textbf{each} $u \in G.V - \{s\}$}
            		\State $u.color \gets$ \texttt{biały}
            		\State $u.d \gets \infty$
            		\State $u.\pi \gets$ \texttt{NIL} 
            	\EndFor
            	\State $s.color \gets$ \texttt{szary}
            	\State $s.d \gets 0$
            	\State $s.\pi \gets$ \texttt{NIL} 
            	\State $Q \gets \varnothing $
            	\State $\Call{Enqueue}{Q, s}$
            	\While{$Q \neq \varnothing$}
            		\State $u \gets \Call{Dequeue}{Q}$
            		\For{\textbf{each} $v \in G.Adj[u]$}
            			\If{$v.color = \texttt{biały}$}
            				\State $v.color \gets \texttt{szary}$
...
\begin{figure}[htpb]
	\centering
	\includegraphics[width=.9\textwidth]{figures/bfs}
	\caption{BFS. Źródło: Cormen et al.}
\end{figure}
    ...
    \begin{algorithmic}[1] % The number tells where the line numbering should start
        \Function{MST-Kruskal}{$G, w$} \algcost{}{//$O(|E|\log |V|)$}
			\State $A \gets \varnothing$
            	...
            	\EndFor         
            	\State posortuj niemalejąco $G.E$ względem wag
            	\For{ \textbf{each} $u \in G.E$ w kolejności niemalejących wag}
            		\If{$\Call{Find-Set}{u} \neq \Call{Find-Set}{v}$}
